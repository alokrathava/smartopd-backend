...
export class OperationsCrmService {
  ...
  async findShifts(
    ...
    if (filters.wardId)
      qb.andWhere('s.wardId = :wardId', { wardId: filters.wardId });
    if (filters.date)
      qb.andWhere('s.shiftDate = :date', { date: filters.date });
    if (filters.staffId)
      qb.andWhere('s.staffId = :staffId', { staffId: filters.staffId });
    if (filters.shiftType)
      qb.andWhere('s.shiftType = :shiftType', { shiftType: filters.shiftType });
    if (filters.status)
  ...
  async getStaffingGaps(
    ...
    for (const [wId, wShifts] of Object.entries(byWard)) {
      const hasNurse = wShifts.some((s) =>
        s.staffRole?.toLowerCase().includes('nurse'),
      );
      if (!hasNurse) {
        gaps.push({
          wardId: wId,
  ...
  async getOvertimeReport(
    ...
    const qb = this.shiftRepo
      ...
      .where('s.facilityId = :facilityId', { facilityId })
      .andWhere(
        '(s.shiftType = :onCall OR (s.actualEndAt IS NOT NULL AND s.actualStartAt IS NOT NULL))',
        {
    ...
    if (filters.from)
      qb.andWhere('s.shiftDate >= :from', { from: filters.from });
    ...
    const report = shifts.map((s) => {
      ...
      if (s.actualStartAt && s.actualEndAt) {
        durationMinutes = dayjs(s.actualEndAt).diff(
          dayjs(s.actualStartAt),
          'minute',
        );
      }
      return {
        ...
        durationMinutes,
        isOvertime:
          s.shiftType === ShiftType.OVERTIME ||
          (durationMinutes !== null && durationMinutes > 480),
  ...
  async updateShiftStatus(
    ...
    if (!shift) throw new NotFoundException('Shift not found');
    shift.status = status;
    if (status === ShiftStatus.IN_PROGRESS && !shift.actualStartAt)
      shift.actualStartAt = new Date();
    if (status === ShiftStatus.COMPLETED && !shift.actualEndAt)
  ...
  async createPreAuth(
    ...
    const pa = this.preAuthRepo.create({
      ...dto,
      facilityId,
  ...
  async updatePreAuth(
    ...
    if (
      ...
      [
        ...
        PreAuthStatus.APPROVED_ENHANCED,
        PreAuthStatus.REJECTED,
        PreAuthStatus.QUERY_RAISED,
      ].includes(dto.status)
    ) {
  ...
  async findPreAuths(
    ...
    if (filters.admissionId)
      qb.andWhere('p.admissionId = :admissionId', {
