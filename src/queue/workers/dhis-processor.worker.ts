...
export class DhisProcessorWorker {
  ...
  async handleAbhaLinkage(job: Job<DhisProcessJobData>) {
    ...
    this.logger.log(
      `Processing ABHA linkage for DHIS: patientId=${patientId}, facilityId=${facilityId}`,
    );

    // Mark patient as ABHA-linked with timestamp
    await this.patientRepo.update({ id: patientId, facilityId }, {
      abhaLinkedAt: new Date(abhaLinkedAt),
    } as any);
  ...
  async handleMonthlyDhisCompute(job: Job<{ facilityId: string }>) {
    ...
    this.logger.log(
      `Computing monthly DHIS incentives for facility ${facilityId}`,
    );
