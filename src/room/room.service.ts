...
export class RoomService {
  constructor(
    ...
    @InjectRepository(HousekeepingLog) private readonly hkRepo: Repository<HousekeepingLog>,
  ) {}

  async createRoom(dto: CreateRoomDto, facilityId: string): Promise<Room> {
    const room = this.roomRepo.create({ ...dto, facilityId });
    return this.roomRepo.save(room);
  }

  async findRooms(facilityId: string, filters: { type?: RoomType; floor?: string; ward?: string }) {
    ...
    if (filters.floor) qb.andWhere('r.floor = :floor', { floor: filters.floor });
  ...
  async createBed(dto: CreateBedDto, facilityId: string): Promise<Bed> {
    const room = await this.roomRepo.findOne({ where: { id: dto.roomId, facilityId } });
  ...
  async getBedsForRoom(roomId: string, facilityId: string): Promise<Bed[]> {
    return this.bedRepo.find({ where: { roomId, facilityId }, order: { bedNumber: 'ASC' } });
  }

  async getBedBoard(facilityId: string) {
    const beds = await this.bedRepo.createQueryBuilder('b')
      .where('b.facilityId = :facilityId', { facilityId })
      .andWhere('b.isActive = true')
      .orderBy('b.roomId', 'ASC').addOrderBy('b.bedNumber', 'ASC')
      .getMany();

    const byStatus = Object.values(BedStatus).reduce((acc, s) => {
  ...
  async getAvailableBeds(facilityId: string, roomType?: RoomType) {
    const qb = this.bedRepo.createQueryBuilder('b')
  ...
  async updateBedStatus(bedId: string, dto: UpdateBedStatusDto, facilityId: string, staffId: string): Promise<Bed> {
  ...
  async startHousekeeping(bedId: string, facilityId: string): Promise<HousekeepingLog> {
    const log = this.hkRepo.create({ bedId, facilityId, startedAt: new Date(), status: HousekeepingStatus.IN_PROGRESS });
    return this.hkRepo.save(log);
  }

  async completeHousekeeping(bedId: string, facilityId: string, staffId: string, notes?: string): Promise<HousekeepingLog> {
    const log = await this.hkRepo.findOne({
      where: { bedId, facilityId, status: HousekeepingStatus.IN_PROGRESS },
      order: { startedAt: 'DESC' },
    });
    if (!log) throw new NotFoundException('No active housekeeping job for this bed');
  ...
  async getHousekeepingHistory(bedId: string, facilityId: string): Promise<HousekeepingLog[]> {
  ...
  async getWardOccupancy(ward: string, facilityId: string) {
    const rooms = await this.roomRepo.find({ where: { ward, facilityId, isActive: true } });
    const roomIds = rooms.map(r => r.id);
    if (roomIds.length === 0) return { ward, totalBeds: 0, occupied: 0, available: 0, occupancyRate: 0 };
  ...
  async getOccupancyDashboard(facilityId: string) {
    const beds = await this.bedRepo.find({ where: { facilityId, isActive: true } });
    const byStatus = Object.values(BedStatus).reduce((acc, s) => {
      acc[s] = beds.filter(b => b.status === s).length;
      return acc;
    }, {} as Record<string, number>);

    const rooms = await this.roomRepo.find({ where: { facilityId, isActive: true } });
    const byType = Object.values(RoomType).reduce((acc, t) => {
      const roomIds = rooms.filter(r => r.type === t).map(r => r.id);
      acc[t] = {
        rooms: roomIds.length,
        beds: beds.filter(b => roomIds.includes(b.roomId)).length,
        occupied: beds.filter(b => roomIds.includes(b.roomId) && b.status === BedStatus.OCCUPIED).length,
      };
      return acc;
    ...
    return {
      ...
      byStatus,
      byRoomType: byType,
      overallOccupancyRate: total > 0 ? Math.round((occupied / total) * 100) : 0,
    };
  }
}
