...
export class AbdmController {
  ...
  @ApiOperation({ summary: 'M1 Step 1: Send OTP to Aadhaar-linked mobile for ABHA creation' })
  generateAadhaarOtp(@Body() dto: GenerateAadhaarOtpDto, @CurrentUser() user: JwtPayload) {
  ...
  @ApiOperation({ summary: 'M1 Step 2: Verify OTP → creates and links ABHA number' })
  verifyAadhaarOtpAndCreateAbha(@Body() dto: VerifyAadhaarOtpDto, @CurrentUser() user: JwtPayload) {
    return this.abdmService.verifyAadhaarOtpAndCreateAbha(dto, user.facilityId!);
  }
  ...
  @HttpCode(HttpStatus.OK)
  @Roles(Role.RECEPTIONIST, Role.NURSE, Role.FACILITY_ADMIN, Role.SUPER_ADMIN)
  @ApiOperation({ summary: 'M2 Step 1: Initiate KYC & health record linking via ABHA number' })
  ...
  confirmM2Link(@Body() dto: ConfirmM2LinkDto, @CurrentUser() user: JwtPayload) {
  ...
  @ApiOperation({ summary: 'M3: Request patient consent to pull ABDM health records' })
  requestConsent(@Body() dto: RequestM3ConsentDto, @CurrentUser() user: JwtPayload) {
  ...
  getPatientHistory(@Param('patientId') patientId: string, @CurrentUser() user: JwtPayload) {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'ABDM webhook: consent grant/deny callbacks from ABDM gateway' })
